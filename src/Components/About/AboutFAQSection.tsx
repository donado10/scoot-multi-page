...
const AboutFAQSectionBig = () => {
  const Template: React.FC<{ title: string; description: string }> = ({
    ...
    return (
      <div className="flex h-fit w-[40rem] flex-col items-center justify-center gap-6 bg-scoot-light px-4 py-8">
        <div className="flex w-full items-center justify-between gap-8">
          <h3 className="font-primary text-xl font-semibold text-scoot-dark">
            {title}
          </h3>
...
const AboutFAQSection = () => {
  ...
  const bigScreen = useMediaQuery(EMediaQuery.SMALL);

  return (
    <>
      {mobileScreen && !bigScreen && <AboutFAQSectionSmall />}
      {bigScreen && <AboutFAQSectionBig />}
    </>
  );
};
